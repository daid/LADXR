...
class Dungeon2:
    def __init__(self, options, world_setup, r):
        entrance = Location(2, name="D2 Entrance")
        Location(2).add(DungeonChest(0x136)).connect(entrance, POWER_BRACELET)  # chest at entrance
        dungeon2_l2 = Location(2, name="D2 Left Door").connect(entrance, AND(KEY2, FOUND(KEY2, 5)))  # towards map chest
        ...
        dungeon2_r2 = Location(2, name="D2 After Torches").connect(entrance, r.fire)
        ...
        dungeon2_r5 = Location(2, name="D2 Pushblock Room").connect(dungeon2_r4, AND(KEY2, FOUND(KEY2, 3)))  # push two blocks together room with owl statue
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
            Location(2).add(OwlStatue(0x12F)).connect(dungeon2_r5, STONE_BEAK2)  # owl statue is before miniboss
        miniboss_room = Location(2, name="D2 Miniboss Room").connect(dungeon2_r5, FEATHER)
        miniboss = Location(2).add(DungeonChest(0x126)).add(DungeonChest(0x121)).connect(miniboss_room, r.miniboss_requirements[world_setup.miniboss_mapping[1]])  # post hinox
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
            Location(2).add(OwlStatue(0x129)).connect(miniboss, STONE_BEAK2)  # owl statue after the miniboss

        dungeon2_ghosts_room = Location(2, name="D2 Boo Buddies Room").connect(miniboss, AND(KEY2, FOUND(KEY2, 5)))
        dungeon2_ghosts_chest = Location(2).add(DungeonChest(0x120)).connect(dungeon2_ghosts_room, OR(r.fire, BOW))  # bracelet chest
        dungeon2_r6 = Location(2, name="D2 After Boo Buddies").add(DungeonChest(0x122)).connect(miniboss, POWER_BRACELET)
        ...
        dungeon2_pre_stairs_boss = Location(2, name="D2 Before Boss Stairs").connect(dungeon2_r6, AND(POWER_BRACELET, KEY2, FOUND(KEY2, 5)))
        dungeon2_post_stairs_boss = Location(2, name="D2 Boss Stairs").connect(dungeon2_pre_stairs_boss, POWER_BRACELET)
        dungeon2_pre_boss = Location(2, name="D2 Outside Boss Room").connect(dungeon2_post_stairs_boss, FEATHER)
        # If we can get here, we have everything for the boss. So this is also the goal room.
        dungeon2_boss_room = Location(2, name="D2 Boss Room").connect(dungeon2_pre_boss, NIGHTMARE_KEY2)
        dungeon2_boss = Location(2).add(HeartContainer(0x12B), Instrument(0x12a)).connect(dungeon2_boss_room, r.boss_requirements[world_setup.boss_mapping[1]])
