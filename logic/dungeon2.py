...
class Dungeon2:
    def __init__(self, options, world_setup, r):
        ...
        Location(dungeon=2).add(DroppedKey(0x132)).connect(dungeon2_r2, AND(r.enemy_requirements["YELLOW_STALFOS"], r.enemy_requirements["GREEN_STALFOS"]))
        Location(dungeon=2).add(DungeonChest(0x137)).connect(dungeon2_r2, AND(KEY2, FOUND(KEY2, 5), r.enemy_requirements["MASK_MIMIC"]))  # compass chest
        ...
        if options.logic == "casual":
            shyguy_key_drop = Location(dungeon=2).add(DroppedKey(0x134)).connect(dungeon2_r3, AND(FEATHER, r.enemy_requirements["MASK_MIMIC"]))  # shyguy drop key
        else:
            shyguy_key_drop = Location(dungeon=2).add(DroppedKey(0x134)).connect(dungeon2_r3, OR(r.rear_attack, AND(FEATHER, r.enemy_requirements["MASK_MIMIC"])))  # shyguy drop key
