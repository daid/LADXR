...
class Dungeon3:
    def __init__(self, options, world_setup, r):
        ...
        area3 = Location("D3 After Zol Chest", dungeon=3).connect(area2, OR(r.enemy_requirements["ZOL"], PEGASUS_BOOTS)) # need to kill slimes to continue or pass through left path
        dungeon3_zol_stalfos = Location(dungeon=3).add(DungeonChest(0x14E)).connect(area3, AND(PEGASUS_BOOTS, r.enemy_requirements["ZOL"], r.enemy_requirements["YELLOW_STALFOS"]))  # 3th chest requires killing the slime behind the crystal pillars

        # now we can go 4 directions,
        area_up = Location("D3 North Room", dungeon=3).connect(area3, AND(KEY3, FOUND(KEY3, 8)))
        dungeon3_north_key_drop = Location(dungeon=3).add(DroppedKey(0x154)).connect(area_up, AND(r.enemy_requirements["GREEN_STALFOS"], r.enemy_requirements["SHROUDED_STALFOS"])) # north key drop
        ...
        Location(dungeon=3).add(DungeonChest(0x144)).connect(area_right, AND(r.enemy_requirements["GREEN_STALFOS"], r.enemy_requirements["ZOL"]))  # map chest
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            area_up.connect(dungeon3_raised_blocks_east, AND(r.throw_pot, r.enemy_requirements["ZOL"]), one_way=True) # use pots to hit the switch

        if options.logic == 'glitched' or options.logic == 'hell':
            area2.connect(dungeon3_raised_blocks_east, AND(r.enemy_requirements["ZOL"], r.super_jump_feather), one_way=True) # use superjump to get over the bottom left block
            ...
            area3.connect(dungeon3_zol_stalfos, AND(r.enemy_requirements["ZOL"], r.enemy_requirements["YELLOW_STALFOS"], r.hookshot_clip_block), one_way=True) # hookshot clip through the northern push block next to raised blocks chest to get to the zol
        ...
        if options.logic == 'hell':
            area2.connect(dungeon3_raised_blocks_east, AND(r.enemy_requirements["ZOL"], r.boots_superhop), one_way=True) # use boots superhop to get over the bottom left block
            ...
            area_up.connect(dungeon3_zol_stalfos, AND(r.super_jump_feather, r.enemy_requirements["ZOL"], r.enemy_requirements["YELLOW_STALFOS"]), one_way=True) # use superjump near top blocks chest to get to zol without boots, keep wall clip on right wall to get a clip on left wall or use obstacles
            ...
            compass_chest.connect(dungeon3_3_bombite_room, AND(r.enemy_requirements["BOMBITE"], OR(BOW, MAGIC_ROD, AND(OR(FEATHER, PEGASUS_BOOTS), OR(SWORD, MAGIC_POWDER)))), one_way=True) # 3 bombite room from the left side, use a bombite to blow open the wall without bombs
