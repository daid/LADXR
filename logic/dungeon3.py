...
class Dungeon3:
    def __init__(self, options, world_setup, r):
        ...
        before_a_stairs = Location("D3 Near First Staircase (Switched)", dungeon=3)
        before_a_stairs_chest3 = Location(dungeon=3).add(DungeonChest(0x14E)) # 200 rupees
        before_a_stairs_chest4 = Location(dungeon=3).add(DungeonChest(0x14C)) # beak
        west_hallway = Location("D3 Long Hallway", dungeon=3)
        center_4way = Location("D3 Four Key Crossroads", dungeon=3)
        ...
        east_4way = Location("D3 East Room", dungeon=3)
        ...
        ledge_pre_pit = Location("D3 Ledge Chest Before Pit", dungeon=3)
        ledge_post_pit = Location("D3 Ledge Chest After Pit", dungeon=3)
        ledge_post_pit_chest_7 = Location(dungeon=3).add(DungeonChest(0x147)) # nightmare key
        miniboss_room = Location("D3 Miniboss Room", dungeon=3)
        after_miniboss_room = Location("D3 Miniboss Reward", dungeon=3)
        ...
        after_pot_door.connect(before_a_stairs, PEGASUS_BOOTS) # TODO: REMOVE THIS, it is logically letting the player into staircase A before they really can
        #TODO: after_pot_door.connect(west_hallway, PEGASUS_BOOTS) # new connection
        #TODO: west_hallway.connect(before_a_stairs, None, one_way=True) # new connection
        slime_room.connect(slime_room_chest2, None)
        slime_room.connect(swordstalfos_entry, r.enemy_requirements["HIDING_ZOL"])
        swordstalfos_room.connect(swordstalfos_room_chest5, None)
        slime_room.connect(before_a_stairs, r.enemy_requirements["HIDING_ZOL"]) # need to kill slimes to continue
        before_a_stairs.connect(center_4way, None) # placeholder for dungeon staircase rando
        before_a_stairs.connect(before_a_stairs_chest3, AND(PEGASUS_BOOTS, r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"]))  # 3rd chest requires killing the slime behind the crystal pillars
        center_4way.connect(south_4way, AND(KEY3, FOUND(KEY3, 8)))
        ...
        center_4way.connect(west_4way, AND(KEY3, FOUND(KEY3, 8)))
        ...
        center_4way.connect(north_4way, AND(KEY3, FOUND(KEY3, 8)))
        ...
        north_4way.connect(before_a_stairs_chest4, AND(r.hit_switch), one_way=True) # after hit switch get zol switch chest
        north_4way.connect(swordstalfos_room, AND(r.hit_switch, r.enemy_requirements["HIDING_ZOL"]), one_way=True)
        center_4way.connect(east_4way, AND(KEY3, FOUND(KEY3, 4)))
        east_4way.connect(main_room, None) # placeholder for dungeon staircase rando
        ...
        ledge_pre_pit.connect(ledge_post_pit, OR(FEATHER, HOOKSHOT))
        ledge_post_pit.connect(ledge_post_pit_chest_7, None)
        ...
        towards_boss4.connect(three_pairodd_room, AND(FEATHER, PEGASUS_BOOTS)) # c passage, dash the red thwomp, then jump to ladder - dungeon staircase rando would replace this logic with entrances
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            north_4way.connect(before_a_stairs_chest4, AND(r.throw_pot), one_way=True) # after hit switch get zol switch chest
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            #TODO: west_hallway.connect(before_a_stairs_chest3, AND(r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"]) # new connection
            #TODO: west_hallway.connect(before_a_stairs_chest4, r.shaq_jump, one_way=True) # depending on switch state
            swordstalfos_entry.connect(swordstalfos_room, r.super_jump_feather) # use superjump to get over the bottom left block
            before_a_stairs.connect(before_a_stairs_chest4, AND(OR(PEGASUS_BOOTS, r.hookshot_clip_block), r.shaq_jump)) #TODO: connect from west hallway for less logic (?) -  boots from south or hookshot clip weith pushblock & zols to get behing the chest, then shaq jump using the pushblock to get on raised platforms
            before_a_stairs.connect(before_a_stairs_chest3, AND(r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"], r.hookshot_clip_block)) # hookshot clip through the northern push block next to raised blocks chest to get to the zol
        ...
        if options.logic == 'hell':
            ...
            #TODO: west_hallway.connect(before_a_stairs_chest4, r.boots_superhop)
            swordstalfos_entry.connect(swordstalfos_room, r.boots_superhop) # use boots superhop to get over the bottom left block
            before_a_stairs.connect(before_a_stairs_chest4, r.boots_superhop, one_way=True) # TODO: REMOVE - connect from west_hallway instead - use boots superhop off top wall or left wall to get on raised blocks
            #TODO: before_a_stairs.connect(west_hallway, r.super_bump)
            #TODO: west_hallway.connect(before_a_stairs_chest3, AND(r.super_bump, r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"]))
            #TODO: before_a_stairs.connect(before_a_stairs_chest4, r.super_bump)
            north_4way.connect(before_a_stairs_chest3, AND(OR(r.hit_switch, r.throw_pot), AND(r.super_jump_feather, r.enemy_requirements["GEL"], r.enemy_requirements["STALFOS_EVASIVE"])), one_way=True) #TODO: move to glitched use superjump near top blocks chest to get to zol without boots, keep wall clip on right wall to get a clip on left wall or use obstacles
            ...
            #TODO: main_room.connect(towards_boss3, r.super_bump, one_way=True)
            #TODO: towards_boss3.connect(after_miniboss_room_chest8, r.super_jump_feather, one_way=True) 
