...
class Dungeon5:
    def __init__(self, options, world_setup, r):
        ...
        compass = Location(dungeon=5).add(DungeonChest(0x19E)).connect(entrance, r.attack_hookshot_powder, id="fv")
        ...
        Location(dungeon=5).add(DungeonChest(0x19B)).connect(area2, r.attack_hookshot_powder, id="fz")  # map chest
        blade_trap_chest = Location(dungeon=5).add(DungeonChest(0x197)).connect(area2, HOOKSHOT, id="g0")  # key chest on the left
        pre_gohma = Location("D5 Before Miniboss", dungeon=5).connect(area2, HOOKSHOT, id="g1") # area top left before keyblock gohma
        ...
        after_stalfos = Location(dungeon=5).add(DungeonChest(0x196)).connect(area2, AND(SWORD, BOMB), id="g6") # Need to defeat master stalfos once for this empty chest; l2 sword beams kill but obscure
        if options.owlstatues == "both" or options.owlstatues == "dungeon":
            butterfly_owl = Location(dungeon=5).add(OwlStatue(0x18A)).connect(after_stalfos, AND(FEATHER, STONE_BEAK5), id="g7")
        else:
            butterfly_owl = None
        after_stalfos.connect(staircase_before_boss, AND(FEATHER, r.attack_hookshot_powder), one_way=True, id="g8") # pathway from stalfos to staircase: past butterfly room and push the block
        ...
        third_arena = Location("D5 Master Stalfos 3", dungeon=5).connect(north_of_crossroads, AND(SWORD, BOMB), id="gd") # can beat 3rd m.stalfos
        stone_tablet = Location(dungeon=5).add(DungeonChest(0x183)).connect(north_of_crossroads, AND(POWER_BRACELET, r.attack_skeleton), id="ge")  # stone tablet
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            after_stalfos.connect(after_keyblock_boss, AND(FEATHER, r.attack_hookshot_powder), id="gm") # circumvent stalfos by going past gohma and backwards from boss door
            if butterfly_owl:
                butterfly_owl.connect(after_stalfos, AND(r.boots_bonk, STONE_BEAK5), id="gn") # boots charge + bonk to cross 2d bridge
            after_stalfos.connect(staircase_before_boss, AND(r.boots_bonk, r.attack_hookshot_powder), one_way=True, id="go") # pathway from stalfos to staircase: boots charge + bonk to cross bridge, past butterfly room and push the block
            ...
            after_keyblock_boss.connect(after_stalfos, AND(FEATHER, r.attack_hookshot_powder), id="gs") # jump from bottom left to top right, skipping the keyblock 
            before_boss.connect(after_stalfos, AND(r.boots_jump, r.attack_hookshot_powder), id="gt") # cross pits room from bottom left to top left with boots jump
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            ...
            after_stalfos.connect(staircase_before_boss, r.super_jump_boots, id="gz") # charge a boots dash in bottom right corner to the right, jump before hitting the wall and use weapon to the left side before hitting the wall
        ...
        if  options.logic == 'hell':
            ...
            after_stalfos.connect(staircase_before_boss, r.super_jump_sword, id="h6") # unclipped superjump in bottom right corner of staircase before boss room, jumping left over the pushable block. reverse is push block
            after_stalfos.connect(staircase_before_boss, r.zoomerang, id="h7") # use zoomerang dashing left to get an unclipped boots superjump off the right wall over the block. reverse is push block
            ...
            before_boss.connect(after_stalfos, r.super_jump_sword, id="hh") # cross pits room from bottom left to top left by unclipped superjump on bottom wall on top of side wall, then jump across            
