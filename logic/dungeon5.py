...
class Dungeon5:
    def __init__(self, options):
        ...
        south_west_bridge_chest = Location(5).add(DungeonChest(0x18E)).connect(north_of_crossroads, OR(HOOKSHOT, AND(FEATHER, PEGASUS_BOOTS))) # south of bridge
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            ...
            south_west_bridge_chest.connect(north_of_crossroads, FEATHER) # tight jump from bottom wall clipped to make it over the pits
        ...
        if  options.logic == 'hell':
            start_hookshot_chest.connect(entrance, OR(PEGASUS_BOOTS, FEATHER)) # use pit buffer to clip into the bottom wall to jump or boots bonk off the wall again
