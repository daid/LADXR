...
class Dungeon7:
    def __init__(self):
        entrance = Location(7)
        Location(7).add(DroppedKey(0x210)).connect(entrance, attack_powder)
        ...
        bottomright_pillar = Location(7).add(DungeonChest(0x211)).connect(topright_pillar_area, OR(FEATHER, attack_hookshot), OR(SHIELD, BRACELET, attack_hookshot))  # compass chest; bracelet can be used in combination with ball; path without feather with hitting switch by falling on the raised blocks
