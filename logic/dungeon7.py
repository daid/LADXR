...
class Dungeon7:
    def __init__(self, options, world_setup, r):
        entrance = Location(7, name="D7 Entrance")
        first_key = Location(7).add(DroppedKey(0x210)).connect(entrance, r.attack_hookshot_powder)
        topright_pillar_area = Location(7, name="D7 Ball Room").connect(entrance, KEY7)
        ...
        bottomleftF2_area = Location(7, name="D7 Hinox Area").connect(topright_pillar_area, r.attack_hookshot)  # area with hinox, be able to hit a switch to reach that area
        ...
        beamos_horseheads_area = Location(7, name="D7 After Boss Door").connect(final_pillar, NIGHTMARE_KEY7) # area behind boss door
        ...
        pre_boss = Location(7, name="D7 Before Boss").connect(beamos_horseheads_area, HOOKSHOT) # raised plateau before boss staircase
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            ...
            topleftF2_area = Location(7, name="D7 Tile Room").connect(topright_pillar_area, r.super_jump_feather) # superjump in top left pillar room over the blocks from right to left, to reach tile room
