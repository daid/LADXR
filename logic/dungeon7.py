...
class Dungeon7:
    def __init__(self, options, world_setup, r):
        ...
        first_key = Location(dungeon=7).add(DroppedKey(0x210)).connect(entrance, r.attack_hookshot_powder, id="ij")
        ...
        three_of_a_kind_north = Location(dungeon=7).add(DungeonChest(0x211)).connect(topright_pillar_area, OR(AND(r.hit_switch, r.attack_hookshot_no_bomb), AND(OR(BOMB, FEATHER), SHIELD)), id="io")  # compass chest; either hit the switch, or have feather to fall on top of raised blocks. No bracelet because ball does not reset
        ...
        final_pillar_area = Location(dungeon=7).add(DungeonChest(0x21C)).connect(bottomleftF2_area, AND(BOMB, HOOKSHOT), id="ix")  # chest that needs to spawn to get to the last pillar
        ...
        if options.logic == 'hard' or options.logic == 'glitched' or options.logic == 'hell':
            three_of_a_kind_north.connect(topright_pillar_area, BOMB, id="j3") # use timed bombs to match the 3 of a kinds (south 3 of a kind room is implicite as normal logic can not reach chest without hookshot)
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            ...
            final_pillar_area.connect(bottomleftF2_area, AND(r.sideways_block_push, OR(r.attack_hookshot, POWER_BRACELET, AND(FEATHER, SHIELD))), id="j9") # sideways block push to get to the chest and pillar, kill requirement for 3 of a kind enemies to access chest. Assumes you do not get ball stuck on raised pegs for bracelet path
            if options.owlstatues == "both" or options.owlstatues == "dungeon":
