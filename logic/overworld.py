...
class World:
    def __init__(self, options, world_setup, r):
        ...
        Location().add(HeartPiece(0x2A4)).connect(mabe_village, r.bush, "0")  # well
        Location().add(FishingMinigame()).connect(mabe_village, AND(r.bush, COUNT("RUPEES", 20)), "1")  # fishing game, heart piece is directly done by the minigame.
        Location().add(Seashell(0x0A3)).connect(mabe_village, r.bush, "2")  # bushes below the shop
        Location().add(Seashell(0x0D2)).connect(mabe_village, PEGASUS_BOOTS, "3")  # smash into tree next to lv1
        Location().add(Song(0x092)).connect(mabe_village, OCARINA, "4")  # Marins song
        Location().add(KeyHole(0x0D3, TAIL_CAVE_OPENED)).connect(mabe_village, TAIL_KEY, "5")  # Marins song
        rooster_cave = Location("Rooster Cave")
        Location().add(DroppedKey(0x1E4)).connect(rooster_cave, AND(OCARINA, SONG3), "6")

        papahl_house = Location("Papahl House")
        mamasha_trade = Location().add(TradeSequenceItem(0x2A6, TRADING_ITEM_RIBBON))
        papahl_house.connect(mamasha_trade, TRADING_ITEM_YOSHI_DOLL, "7")
    ...
    def _createShopSanity(self, options, index, room, id=None):
        ...
        Location().add(ShopItem(0, room=room)).connect(shop, FOUND("RUPEES", 100 + 100 * index), f"{id}-0")
        Location().add(ShopItem(1, room=room)).connect(shop, FOUND("RUPEES", 150 + 100 * index), f"{id}-1")
        return shop
