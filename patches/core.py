...
def noSwordMusic(rom):
    ...
    rom.patch(2, 0x3B42, ASM("ld a, [$DB4E]"), ASM("ld a, $01"), fill_nop=True)
    rom.patch(3, 0x0994, ASM("ld a, [$DB4E]"), ASM("ld a, $01"), fill_nop=True)

def removeGhost(rom):
    ## Ghost patch
    # Do not have the ghost follow you after dungeon 4
    rom.patch(0x03, 0x1E0A, ASM("LD [$DB79], A"), "", fill_nop=True)
